        ...
        <Card className="p-6 h-full">
          ...
          <div className="space-y-6">
            ...
            <div className="flex space-x-4">
              <Button className="h-12 px-6 flex-1">保存配置</Button>
              <Button variant="outline" className="h-12 px-6 flex-1">重置默认</Button>
            </div>
          </div>
        </Card>
      </div>

      {/* Right Column - Outlet Mapping */}
          ...
          <div className="grid grid-cols-4 gap-4 h-full">
            {Array.from({ length: parseInt(outletCount) }, (_, i) => i + 1).map((outlet) => (
              <div key={outlet} className="space-y-3">
                <div className={`h-20 outlet-${outlet} rounded-lg flex items-center justify-center`}>
                  <span className="text-white font-bold text-lg">出口 {outlet}</span>
                </div>
            ...
            <div className="space-y-3">
              <div className="h-20 outlet-reject rounded-lg flex items-center justify-center">
